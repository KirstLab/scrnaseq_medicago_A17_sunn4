...
generate_dotplots_by_time <- function(file_name = file_name,
    ...
    for( i in unique(markers$Group) ) {
        ...
        ggsave(filename = paste0("images/dotplots_by_time/",folder_n,"/",
                                 ...
                                 i, ".svg"),
               plot = dotp,
               width = round(3 + round(3 + nrow(markers2)*0.5)),
...
generate_dotplots_by_time(file_name = "RECLUSTERING/PERICYCLE/rds_file_subset/seurat_formated_pericycle",
                          c_nickname = "PE",
                          output_name = "seurat_formated_pericycle",
                          folder_n = "pericycle_Schiessl")

generate_heatmaps_by_time(file_name = "RECLUSTERING/PERICYCLE/rds_file_subset/medicago_integrated_subset_pericycle",
                          c_nickname = "PE",
                          output_name = "seurat_formated_pericycle",
                          folder_n = "pericycle_Schiessl")
...
markers <- suppressMessages(
    vroom("/Users/wendellpereira/Desktop/Table_S3_DEGs_across_time.txt",
          delim = "\t",
          col_names = T,
          na = "NA") )
