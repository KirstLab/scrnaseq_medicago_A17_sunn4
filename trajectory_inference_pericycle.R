...
cds <- readRDS("RECLUSTERING/PERICYCLE/rds_file_subset/medicago_integrated_subset_pericycle.rds")
...
foldername1 <- "RECLUSTERING/PERICYCLE/"
saveRDS(sds, paste0(foldername1,
                    "rds_file_subset/sunn4_trajectory_pericycle.rds") )
...
foldername2 <-  paste0(foldername1, "dynverse_markers_pericycle/images/")
system( paste0( "mkdir -p ", foldername2) )
ggplot2::ggsave(paste0(foldername2, "pericycle_trajectory_dynverse_grouping.svg"),
                p,
                height=12,
                width=16,
                units="in",
...
ggplot2::ggsave(paste0(foldername2, "pericycle_trajectory_dynverse_dendro.svg"),
                p1,
                height=12,
                width=16,
                units="in",
...
foldername3 <- paste0(foldername1, "dynverse_markers_pericycle/")
...
write.table(feat_importances_annot,
            paste0(foldername3, "pericycle_trajectory_dynverse_whole_trajectory.tsv"),
            col.names = T,
            row.names = F,
            quote = F,
            sep = "\t") 
...
saveRDS(sce_fitted,
        file = paste0("RECLUSTERING/PERICYCLE/rds_file_subset/TRADEseq_cortex_nodules.rds")
)
...
foldername4 <- paste0(foldername1, "tradeseq_markers_pericycle/")
system( paste0("mkdir -p ", foldername4) )
write.table( feat_importances2,
             paste0( foldername4, "TRADEseq_pericycle.csv" ),
             col.names = T,
             row.names = F,
             sep = ",",
             quote = F
...
foldername5 <- "RECLUSTERING/PERICYCLE/tradeseq_markers_pericycle/images/"
...
write.table(averaged_heatdata_scaled,
            paste0(foldername4, "expression_of_DEGs_within_trajectory_pericycle_IT.tst"),
