...
cds <- readRDS("RECLUSTERING/Epidermis_roothair/rds_file_subset/medicago_integrated_subset_epidermis.rds")
...
cds <- cds[, colnames(cds) %in% cells_on_clusters ]
...
saveRDS(sds, paste0(folder_name,"roothair_IT_trajectory.rds") )
...
folder_name2 <- "RECLUSTERING/Epidermis_roothair/dynverse_markers_epidermis_roothair/images/"
system( paste0( "mkdir -p ", folder_name2) )
ggplot2::ggsave( paste0(folder_name2,
                        "root_hair_IT_trajectory_dynverse_grouping.svg"),
                 p,
                 height=12,
                 width=16,
                 units="in",
...
ggplot2::ggsave( paste0(folder_name2,
                        "root_hair_IT_trajectory_dynverse_dendro.svg"),
                 p1,
                 height=12,
                 width=16,
                 units="in",
...
foldername3 <- paste0(folder_name, "dynverse_markers_roothair_IT/")
...
write.table(feat_importances_annot,
            paste0(foldername3,
                   "root_hair_IT_trajectory_dynverse_whole_trajectory.tsv"),
            col.names = T,
            row.names = F,
            quote = F,
            sep = "\t")
...
saveRDS(sce_fitted,
        file = paste0("RECLUSTERING/Epidermis_roothair/rds_file_subset/TRADEseq_root_hair_IT.rds")
)
...
foldername4 <- "RECLUSTERING/Epidermis_roothair/tradeseq_markers_root_hair_IT/"
system( paste0("mkdir -p ", folder_name4) )

write.table( feat_importances2,
             paste0(foldername4,
                    "TRADEseq_roothair_IT.csv"),
             col.names = T,
             row.names = F,
             sep = ",",
             quote = F
...
heatdata_scaled <- as.data.frame(heatdata_scaled)
...
foldername5 <- "RECLUSTERING/Epidermis_roothair/tradeseq_markers_root_hair_IT/images"
system( paste0("mkdir -p ", foldername5) )
ggplot2::ggsave(
    paste0(foldername5,
           "heatmap_significant_genes_tradseq_top100_trajectory.svg"),
    p_heatmap,
...
ggplot2::ggsave(
    paste0(foldername5,
           "heatmap_significant_genes_tradseq_top100_trajectory.png"),
    p_heatmap,
    height= nrow(heatdata_scaled)*0.2,
    width=20,
    units="in",
...
ggplot2::ggsave(
    paste0(foldername5,
           "/bined_heatmap_significant_genes_tradseq_top100_trajectory.svg"),
    bined_heatmap,
    height= nrow(heatdata_scaled)*0.2,
    width=16,
    units="in",
...
ggplot2::ggsave(    
    paste0(foldername5,
           "/bined_heatmap_significant_genes_tradseq_top100_trajectory.png"),
    bined_heatmap,
    height= nrow(heatdata_scaled)*0.2,
    width=16,
    units="in",
...
write.table(averaged_heatdata_scaled,
            paste0(foldername4, "expression_of_DEGs_within_trajectory_roothair_IT.tst"),
